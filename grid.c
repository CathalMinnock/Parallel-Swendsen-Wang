...
void magnetization() {
	...
	if(rank == root) {
		mag = sqrt(total_x_sum * total_x_sum + total_y_sum * total_y_sum); // / (x_size * y_size * z_size);
	}
}
...
// needs to know labels and bonds, updates labels
// reduce cluster labels across boundaries
// if the processor is not at the end of the total lattice, then the label in the boundary will be less than the corresponding label in the lattice, since it's strictly further from the origin. So we don't need to check
// if the processor is at the end of the lattice, then the opposite is true, because of periodic boundary conditions.
void reduce_boundary_labels() {
	int i, j, k;
	// Bottom boundary
	if(cart_coordinates[0] == cart_dims[0] - 1) {
		for(j = 0; j < local_y_size; ++j)
		for(k = 0; k < local_z_size; ++k) {
	...
	// Right boundary
	if(cart_coordinates[1] == cart_dims[1] - 1) {
		for(i = 0; i < local_x_size; ++i)
		for(k = 0; k < local_z_size; ++k) {
	...
	// Front boundary
	if(cart_coordinates[2] == cart_dims[2] - 1) {
		for(i = 0; i < local_x_size; ++i)
		for(j = 0; j < local_y_size; ++j) {
...
void print_grid_labels(point ***grid, int x, int y, int z) {
	int i,j,k;
	for(k=0; k < local_z_size; ++k) {
		printf("========== k = %i ==========\n", k);
		for(i=0; i < local_x_size; ++i){
